...
async function cargarDatosSociales() {
    ...
    if (cont) {
        if (comentarios && comentarios.length > 0) {
            ...
            cont.innerHTML = '<p style="color:#555; text-align:center;">No hay comentarios técnicos aún.</p>';
        }
    }
}
...
async function enviarComentarioAnalisis() {
    ...
    try {
        ...
        const { error } = await clienteSupabase.from('comunidad_comentarios').insert([{ 
            repertorio_id: idProcesion,
